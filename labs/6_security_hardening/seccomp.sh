...
PROFILES="audit.json violation.json fine-grained.json"
PROFILE_DESCRIPTIONS="audit.json|logs all syscalls fine-grained.json|allows specific syscalls violation.json|blocks all syscalls"

for PROFILE in $PROFILES; do
    DESCRIPTION=$(echo "$PROFILE_DESCRIPTIONS" | grep -o "${PROFILE}|[^|]*" | cut -d'|' -f2)
    log_info "Command: curl -L -o $PROFILES_PATH/$PROFILE $BASE_URL/$PROFILE"
    if curl -L -o "$PROFILES_PATH/$PROFILE" "$BASE_URL/$PROFILE"; then
        log_success "Downloaded $PROFILE ($DESCRIPTION)"
    else
